...
func (s *resultService) GetByUser(ctx context.Context, id string) ([]models.Result, error) {
...
func (s *resultService) Keirsey(ctx context.Context, id string, res [4]int) error {
	...
	keirsey := &models.TestKeirsey{Res: fmt.Sprintf("%s, %s", profile, tp), UserID: id, Date: time.Now().String()}
...
func (s *resultService) Hall(ctx context.Context, id string, res [5]int) error {
	...
	hall := &models.TestHall{Res: temp, UserID: id, Date: time.Now().String()}
	return s.repo.Add(ctx, hall)
}

func (s *resultService) Bass(ctx context.Context) error {
	bass := &models.TestBass{}
